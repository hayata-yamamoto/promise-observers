...
const capturePromiseResult = async <T>(observable: ObservableType, promise: {key: string; fn: () => Promise<T>}): Promise<T> => {
...
export const promiseAllWithObservers = async <T>(promises: {key: string; fn: () => Promise<T>}[], observers: ObserverType[]): Promise<T[]> => {
    const _observable = observable()

    observers.forEach(observer => _observable.on(observer))
    const ret = await Promise.all(promises.map(promise => capturePromiseResult(_observable, promise)))
